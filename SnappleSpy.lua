...
local Background = Create("Frame",{Parent = SimpleSpy3,BackgroundColor3 = Color3.new(1, 0, 0),BackgroundTransparency = 0.8,Position = UDim2.new(0, 500, 0, 200),Size = UDim2.new(0, 450, 0, 268)})
...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 75, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "SnappleSpy",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
newButton(
    "Spam Remote",
    function() return "Click to spam the selected remote using RunService (fires every frame)" end,
    function()
        ...
        local stopConnection = StopButton.MouseButton1Click:Connect(function()
            ...
            StopButton:Destroy()
            TextLabel.Text = "Spam stopped!"
        end)
        
        TextLabel.Text = "Spamming " .. Remote.Name .. "... Click Stop Spam to stop"
        ...
        connection = RunService.Heartbeat:Connect(fireRemote)
        ...
        StopButton.AncestryChanged:Connect(function()
            if not StopButton.Parent then
                cleanup()
            end
        end)
        
        local shutdownConnection
        shutdownConnection = SimpleSpy3.AncestryChanged:Connect(function()
            if not SimpleSpy3.Parent then
                cleanup()
                if shutdownConnection then
